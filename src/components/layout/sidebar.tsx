...
import {
  ...
  Activity, Terminal, FileEdit, SlidersHorizontal, Link as LinkIcon, Chrome, LifeBuoy,
} from "lucide-react";
...
const ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  ShieldCheck, Home, History, Swords, Bot, Wrench, Search, Link2, PenLine, Network, CalendarClock, Share2, BarChart3, Mail, Activity, Terminal, FileEdit, SlidersHorizontal, Link: LinkIcon, Chrome, LifeBuoy,
};
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-0.5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-muted-foreground/30">
        ...
        {ADMIN_VIEWS.filter((v) => !["admin", "settings", "billing", "integrations"].includes(v.key)).map((v) => {
          ...
          const alwaysEnabled = ["admin", "history", "competitors", "ai-chat", "tools", "ufuqlink", "extension", "support", "keywords", "backlinks", "content", "link-graph", "scheduled", "portal", "search-console", "email", "activity", "api-docs", "page-editor", "crawl-settings"].includes(v.key);
